...
- 图表文件必须保存到当前执行输出路径下的 `plot` 子目录中。完整的保存路径结构为 `{output_path}/{model}/{date}/plot/文件名.png`，其中 `{output_path}` 是执行时提供的基础输出路径，`{model}` 和 `{date}` 是当前执行的上下文目录。执行者应该能够从环境中获取或构建这个基本路径。
...
不要在本地文件系统中保存任何内容，除非是指示保存生成的图表文件。图表应保存到指定的 `plot` 输出子目录中。其他中间结果应缓存到IPython Kernel中。代码单元中绝不使用bash命令。
